...
class ParallelConfig:
    ...
    enable_adaptive_ubatch: bool = False
    """Enable runtime selection of the micro-batch count.

    When enabled, the worker may choose a smaller effective micro-batch count
    than `ubatch_size` for each scheduled batch based on model size and the
    current prefill/decode mix. Backends that do not support adaptive
    micro-batching may ignore this option.
    """
    adaptive_ubatch_max_size: int = 4
    """Maximum micro-batch count considered by adaptive micro-batching."""
    adaptive_ubatch_min_gain_pct: float = 5.0
    """Minimum predicted gain required before adaptive micro-batching is used."""
    adaptive_ubatch_use_analytical_prior: bool = True
    """Use the fitted analytical cost model as the adaptive cold-start prior."""
    adaptive_ubatch_prefill_threshold_pct: float = 0.0
    """Minimum prefill-token ratio before adaptive policy considers M > 1.

    Defaults to 0 so decode and mixed batches can still probe M=2; set a
    positive value to restore prefill-heavy-only gating.
    """
    adaptive_ubatch_warmup_steps: int = 8
    """Deprecated alias for adaptive_ubatch_min_observations."""
    adaptive_ubatch_min_observations: int = 8
    """Minimum observations per (workload bucket, M) before trusting calibration."""
    adaptive_ubatch_explore_pct: float = 5.0
    """Deprecated compatibility knob; exploration is controlled by enable/interval."""
    adaptive_ubatch_enable_exploration: bool = True
    """Enable deterministic, interval-gated controlled exploration."""
    adaptive_ubatch_switch_threshold_pct: float = 5.0
    """Minimum measured score improvement required to switch micro-batch count."""
    adaptive_ubatch_bad_threshold_pct: float = 20.0
    """Measured score regression threshold before a choice is marked bad."""
    adaptive_ubatch_cooldown_steps: int = 32
    """Decision steps before a bad-performance candidate can be retried."""
    adaptive_ubatch_failure_cooldown_steps: int = 32
    """Decision steps before an execution-failure candidate can be retried."""
    adaptive_ubatch_ewma_alpha: float = 0.2
    """EWMA alpha for online micro-batch cost estimation."""
    adaptive_ubatch_disable_m4_for_large_model: bool = False
    """Disable M=4 exploration for large models by default."""
    adaptive_ubatch_min_tokens_m2: int = 128
    """Minimum scheduled tokens required before M=2 is considered."""
    adaptive_ubatch_min_tokens_m4: int = 512
    ...
    """Minimum decision steps to hold a selected M before switching."""
    adaptive_ubatch_switch_confirmations: int = 2
    """Consecutive wins required before switching to another M."""
    adaptive_ubatch_feedback_interval_steps: int = 64
    """Decision-step interval for sampled cross-stage critical-path feedback."""
    adaptive_ubatch_candidate_calibration_observations: int = 3
    ...
    def _validate_parallel_config(self) -> Self:
        ...
        if self.adaptive_ubatch_max_size < 1:
            raise ValueError("adaptive_ubatch_max_size must be >= 1.")
        if self.adaptive_ubatch_min_gain_pct < 0:
            raise ValueError("adaptive_ubatch_min_gain_pct must be >= 0.")
        if self.adaptive_ubatch_prefill_threshold_pct < 0:
            raise ValueError("adaptive_ubatch_prefill_threshold_pct must be >= 0.")
        if self.adaptive_ubatch_warmup_steps < 0:
            raise ValueError("adaptive_ubatch_warmup_steps must be >= 0.")
        if self.adaptive_ubatch_min_observations < 0:
            raise ValueError("adaptive_ubatch_min_observations must be >= 0.")
        if self.adaptive_ubatch_explore_pct < 0:
            raise ValueError("adaptive_ubatch_explore_pct must be >= 0.")
        if self.adaptive_ubatch_switch_threshold_pct < 0:
            raise ValueError("adaptive_ubatch_switch_threshold_pct must be >= 0.")
        if self.adaptive_ubatch_bad_threshold_pct < 0:
            raise ValueError("adaptive_ubatch_bad_threshold_pct must be >= 0.")
        if self.adaptive_ubatch_cooldown_steps < 1:
            raise ValueError("adaptive_ubatch_cooldown_steps must be >= 1.")
        if self.adaptive_ubatch_failure_cooldown_steps < 1:
            raise ValueError("adaptive_ubatch_failure_cooldown_steps must be >= 1.")
        if not 0 < self.adaptive_ubatch_ewma_alpha <= 1:
            raise ValueError("adaptive_ubatch_ewma_alpha must be in (0, 1].")
        if self.adaptive_ubatch_mode not in {
            "analytical_only",
            "calibrated",
        ...
        if self.adaptive_ubatch_max_uncertainty_ratio < 0:
            raise ValueError("adaptive_ubatch_max_uncertainty_ratio must be >= 0.")
        if self.adaptive_ubatch_cold_start_penalty_ratio < 0:
            raise ValueError("adaptive_ubatch_cold_start_penalty_ratio must be >= 0.")
        if self.adaptive_ubatch_max_correction_ratio < 0:
            raise ValueError("adaptive_ubatch_max_correction_ratio must be >= 0.")
        if self.adaptive_ubatch_max_calibration_scale < 1:
            raise ValueError("adaptive_ubatch_max_calibration_scale must be >= 1.")
        if self.adaptive_ubatch_min_hold_steps < 0:
            raise ValueError("adaptive_ubatch_min_hold_steps must be >= 0.")
        if self.adaptive_ubatch_switch_confirmations < 1:
            raise ValueError("adaptive_ubatch_switch_confirmations must be >= 1.")
        if self.adaptive_ubatch_feedback_interval_steps < 1:
            raise ValueError(
                "adaptive_ubatch_feedback_interval_steps must be >= 1."
            )
        if self.adaptive_ubatch_candidate_calibration_observations < 0:
            raise ValueError(
                "adaptive_ubatch_candidate_calibration_observations must be >= 0."
            )
        if self.adaptive_ubatch_safe_m < 1:
            raise ValueError("adaptive_ubatch_safe_m must be >= 1.")
        if self.adaptive_ubatch_exploration_interval_steps < 1:
            raise ValueError(
                "adaptive_ubatch_exploration_interval_steps must be >= 1."
            )
        if self.adaptive_ubatch_exploration_stable_steps < 1:
            raise ValueError(
                "adaptive_ubatch_exploration_stable_steps must be >= 1."
            )
        ...
        if self.adaptive_ubatch_min_tokens_m4 < 1:
            raise ValueError("adaptive_ubatch_min_tokens_m4 must be >= 1.")
        if not 0 <= self.adaptive_ubatch_min_prefill_ratio_m4 <= 1:
            raise ValueError(
                "adaptive_ubatch_min_prefill_ratio_m4 must be in [0, 1]."
            )
    ...
    def use_ubatching(self) -> bool:
        # Keep this property scoped to vLLM's native DBO/static microbatching.
        # Ascend PP adaptive microbatching uses enable_adaptive_ubatch directly;
        # treating it as native ubatching triggers unrelated DeepEP all2all
        # backend validation during VllmConfig construction.
