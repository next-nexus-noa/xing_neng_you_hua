...
from vllm.benchmarks.power import calculate_power_metrics, print_power_metrics
...
def add_cli_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to a power sample jsonl file generated by "
        "scripts/power_monitor.py sample. When set, power and energy "
        "metrics are calculated and added to the benchmark result JSON.",
    )
    parser.add_argument(
        "--ignore-eos",
        action="store_true",
...
async def main_async(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.power_metrics_file:
        try:
            power_metrics = calculate_power_metrics(
                args.power_metrics_file, benchmark_result
            )
            result_json.update(power_metrics)
            result_json["power_metrics_file"] = args.power_metrics_file
            print_power_metrics(power_metrics)
        except Exception as e:
            warnings.warn(f"Failed to calculate power metrics: {e}", stacklevel=2)
