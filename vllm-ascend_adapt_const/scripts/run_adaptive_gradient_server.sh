...
ADAPTIVE_UBATCH_COLD_START_PENALTY_RATIO="${ADAPTIVE_UBATCH_COLD_START_PENALTY_RATIO:-0.25}"
...
ADAPTIVE_UBATCH_ONLINE_RESIDUAL_LIMIT_PCT="${ADAPTIVE_UBATCH_ONLINE_RESIDUAL_LIMIT_PCT:-8}"
ADAPTIVE_UBATCH_VALIDATION_STAGE_STEPS="${ADAPTIVE_UBATCH_VALIDATION_STAGE_STEPS:-4,8,16,32}"
ADAPTIVE_UBATCH_VALIDATION_SAFE_STEPS="${ADAPTIVE_UBATCH_VALIDATION_SAFE_STEPS:-8}"
ADAPTIVE_UBATCH_VALIDATION_MIN_OUTPUT_TOKENS="${ADAPTIVE_UBATCH_VALIDATION_MIN_OUTPUT_TOKENS:-8}"
ADAPTIVE_UBATCH_VALIDATION_GAIN_PCT="${ADAPTIVE_UBATCH_VALIDATION_GAIN_PCT:-2}"
ADAPTIVE_UBATCH_VALIDATION_REQUIRED_OBSERVATIONS="${ADAPTIVE_UBATCH_VALIDATION_REQUIRED_OBSERVATIONS:-2}"
ADAPTIVE_UBATCH_VALIDATION_CONFIDENCE_KAPPA="${ADAPTIVE_UBATCH_VALIDATION_CONFIDENCE_KAPPA:-1.0}"
ADAPTIVE_UBATCH_VALIDATION_MIN_TARGET_SHARE="${ADAPTIVE_UBATCH_VALIDATION_MIN_TARGET_SHARE:-0.5}"
ADAPTIVE_UBATCH_VALIDATION_WASHOUT_STEPS="${ADAPTIVE_UBATCH_VALIDATION_WASHOUT_STEPS:-1}"
ADAPTIVE_UBATCH_QUEUE_AGE_GROWTH_THRESHOLD_MS="${ADAPTIVE_UBATCH_QUEUE_AGE_GROWTH_THRESHOLD_MS:-10}"
# Per-step JSON traces are diagnostic instrumentation. Keep them out of the
# formal performance path unless a run explicitly opts in.
ADAPTIVE_DECISION_TRACE_ENABLED="${ADAPTIVE_DECISION_TRACE_ENABLED:-0}"
...
tail -n +2 "$PLAN" | while IFS=, read -r run_id scenario_id dataset scenario model_label model request_rate burstiness seed num_prompts micro_batch max_model_len host port pp_size tp_size dtype dataset_name dataset_path dataset_extra_args; do
  ...
  if [[ "$server_mode" == "adaptive" ]]; then
    server_args=(
      ...
      --adaptive-ubatch-online-residual-limit-pct "$ADAPTIVE_UBATCH_ONLINE_RESIDUAL_LIMIT_PCT"
      --adaptive-ubatch-validation-stage-steps "$ADAPTIVE_UBATCH_VALIDATION_STAGE_STEPS"
      --adaptive-ubatch-validation-safe-steps "$ADAPTIVE_UBATCH_VALIDATION_SAFE_STEPS"
      --adaptive-ubatch-validation-min-output-tokens "$ADAPTIVE_UBATCH_VALIDATION_MIN_OUTPUT_TOKENS"
      --adaptive-ubatch-validation-gain-pct "$ADAPTIVE_UBATCH_VALIDATION_GAIN_PCT"
      --adaptive-ubatch-validation-required-observations "$ADAPTIVE_UBATCH_VALIDATION_REQUIRED_OBSERVATIONS"
      --adaptive-ubatch-validation-confidence-kappa "$ADAPTIVE_UBATCH_VALIDATION_CONFIDENCE_KAPPA"
      --adaptive-ubatch-validation-min-target-share "$ADAPTIVE_UBATCH_VALIDATION_MIN_TARGET_SHARE"
      --adaptive-ubatch-validation-washout-steps "$ADAPTIVE_UBATCH_VALIDATION_WASHOUT_STEPS"
      --adaptive-ubatch-queue-age-growth-threshold-ms "$ADAPTIVE_UBATCH_QUEUE_AGE_GROWTH_THRESHOLD_MS"
      "${adaptive_trace_args[@]}"
      "$large_model_m4_arg"
    )
  fi

  {
    ...
    echo "adaptive_ubatch_online_residual_limit_pct=$ADAPTIVE_UBATCH_ONLINE_RESIDUAL_LIMIT_PCT"
    echo "adaptive_ubatch_validation_stage_steps=$ADAPTIVE_UBATCH_VALIDATION_STAGE_STEPS"
    echo "adaptive_ubatch_validation_safe_steps=$ADAPTIVE_UBATCH_VALIDATION_SAFE_STEPS"
    echo "adaptive_ubatch_validation_min_output_tokens=$ADAPTIVE_UBATCH_VALIDATION_MIN_OUTPUT_TOKENS"
    echo "adaptive_ubatch_validation_gain_pct=$ADAPTIVE_UBATCH_VALIDATION_GAIN_PCT"
    echo "adaptive_ubatch_validation_required_observations=$ADAPTIVE_UBATCH_VALIDATION_REQUIRED_OBSERVATIONS"
    echo "adaptive_ubatch_validation_confidence_kappa=$ADAPTIVE_UBATCH_VALIDATION_CONFIDENCE_KAPPA"
    echo "adaptive_ubatch_validation_min_target_share=$ADAPTIVE_UBATCH_VALIDATION_MIN_TARGET_SHARE"
    echo "adaptive_ubatch_validation_washout_steps=$ADAPTIVE_UBATCH_VALIDATION_WASHOUT_STEPS"
    echo "adaptive_ubatch_queue_age_growth_threshold_ms=$ADAPTIVE_UBATCH_QUEUE_AGE_GROWTH_THRESHOLD_MS"
