...
ADAPTIVE_UBATCH_MODE="${ADAPTIVE_UBATCH_MODE:-calibrated_risk_aware}"
...
ADAPTIVE_UBATCH_EXPLORATION_INTERVAL_STEPS="${ADAPTIVE_UBATCH_EXPLORATION_INTERVAL_STEPS:-64}"
...
ADAPTIVE_UBATCH_POLICY_PROFILE="${ADAPTIVE_UBATCH_POLICY_PROFILE:-best_20260729}"
ADAPTIVE_UBATCH_FEEDBACK_REDUCTION="${ADAPTIVE_UBATCH_FEEDBACK_REDUCTION:-sync}"
...
tail -n +2 "$PLAN" | while IFS=, read -r run_id scenario_id dataset scenario model_label model request_rate burstiness seed num_prompts micro_batch max_model_len host port pp_size tp_size dtype dataset_name dataset_path dataset_extra_args; do
  ...
  env_args=(
    ...
    "ADAPTIVE_UBATCH_POLICY_PROFILE=$ADAPTIVE_UBATCH_POLICY_PROFILE"
    "ADAPTIVE_UBATCH_FEEDBACK_REDUCTION=$ADAPTIVE_UBATCH_FEEDBACK_REDUCTION"
  )
...

  {
    ...
    echo "adaptive_ubatch_policy_profile=$ADAPTIVE_UBATCH_POLICY_PROFILE"
    echo "adaptive_ubatch_feedback_reduction=$ADAPTIVE_UBATCH_FEEDBACK_REDUCTION"
