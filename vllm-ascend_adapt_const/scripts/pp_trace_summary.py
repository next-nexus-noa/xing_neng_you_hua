...
def summarize_grouping(
    ...
    rank0 = sorted(
        (
            rec
            for rec in timelines
            if int(rec.get("pp_rank", -1)) == 0
        ),
        key=lambda rec: int(rec.get("step", -1)),
    )[skip:]
    scom = [
        rec["scom_grouping"]
        for rec in rank0
        if rec.get("scom_grouping") is not None
    ]
    compute_aware = [
        rec["compute_aware_grouping"]
        for rec in rank0
        if rec.get("compute_aware_grouping") is not None
    ]
    if not scom and not compute_aware:
        return "No grouping metadata after applying --skip.\n"

    records = scom if scom else compute_aware
    mode = "SCOM" if scom else "compute-aware"
    applied = [record for record in records if record.get("applied")]
    reasons = Counter(
        str(record.get("reason", "unknown")) for record in records
    )
    gains = [
        float(record.get("predicted_gain", 0.0))
        for record in records
    ]
    overhead = [
        float(record.get("decision_overhead_us", 0.0))
        for record in records
        if "decision_overhead_us" in record
    ]
    lines = [
        f"{mode} grouping summary",
        f"  planned/applied     : {len(records)} / {len(applied)} "
        f"({len(applied) / len(records) * 100.0:.2f}%)",
        f"  predicted gain avg  : {avg(gains) * 100.0:.2f}%",
        f"  decision overhead   : {avg(overhead):.1f} us",
        "  reasons             : "
        + ", ".join(
            f"{reason}={count}"
    ...
    if scom:
        sources = Counter(
            str(record.get("selection_source", "unknown"))
            for record in records
        )
        cache_hits = sum(
        ...
        capacity_changed = [
            record
            for record in records
            if record.get("selected_capacities")
            != record.get("baseline_capacities")
        ]
        candidate_counts = [
            float(record.get("capacity_candidate_count", 0))
            for record in records
        ]
        bucket_rejections = [
            float(record.get("shape_bucket_rejection_count", 0))
            for record in records
        ]
        lines.extend(
            [
                ...
                "  plan cache hits     : "
                f"{cache_hits} / {len(records)}",
                "  capacity changed    : "
                f"{len(capacity_changed)} / {len(records)}",
                "  capacity candidates : "
                f"{avg(candidate_counts):.2f} avg",
                "  bucket rejections   : "
                f"{avg(bucket_rejections):.2f} avg",
            ]
        )
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    print(summarize_grouping(timelines, args.skip))
