...
CAPACITY_PUBLISH="${CAPACITY_PUBLISH:-0}"
...
case "$CAPACITY_SOURCE" in
  local)
    if ! grep -q ',capacity_probe,' "$PLAN" && grep -q ',auto-' "$PLAN"; then
      echo "A formal-only auto-QPS plan requires CAPACITY_SOURCE=shared." >&2
      exit 1
    fi
  ...
  shared)
    ...
    if [[ "$CAPACITY_PUBLISH" == "1" ]]; then
      echo "CAPACITY_SOURCE=shared cannot be combined with CAPACITY_PUBLISH=1." >&2
      exit 1
    fi
    if grep -q ',capacity_probe,' "$PLAN"; then
      echo "CAPACITY_SOURCE=shared requires a formal-only adaptive plan." >&2
      exit 1
    fi
    ;;
  *)
...
publish_capacity() {
  ...
  python "$CAPACITY_EXCHANGE_SCRIPT" publish \
    ...
    --run-id "$run_id" \
    --low-factor "$LOW_QPS_FACTOR" \
    --medium-factor "$MEDIUM_QPS_FACTOR" \
    --high-factor "$HIGH_QPS_FACTOR"
}
...
tail -n +2 "$PLAN" | while IFS=, read -r run_id scenario_id dataset scenario model_label model request_rate burstiness seed num_prompts micro_batch max_model_len host port pp_size tp_size dtype dataset_name dataset_path dataset_extra_args; do
  ...
  if [[ "$request_rate_spec" == auto-* ]]; then
    ...
    if [[ "$resolve_status" -ne 0 || -z "$request_rate" ]]; then
      if [[ "$CAPACITY_SOURCE" == "shared" ]]; then
        echo "[$adaptive_id] Could not resolve $request_rate_spec from shared session $CAPACITY_SESSION."
      else
        echo "[$adaptive_id] Could not resolve $request_rate_spec from $CAPACITY_FILE."
      fi
      date -Is > "$run_dir/client.failed"
      date -Is > "$run_dir/client.done"
      continue
    fi
    echo "[$adaptive_id] Resolved request rate: $request_rate_spec -> $request_rate req/s."
  fi

  echo "[$adaptive_id] Running benchmark."
  {
    ...
    echo "capacity_publish=$CAPACITY_PUBLISH"
  ...
  if (( WARMUP_RUNS > 0 )); then
    for warmup_idx in $(seq 1 "$WARMUP_RUNS"); do
      ...
      if [[ "$warmup_status" -ne 0 ]]; then
        ...
        date -Is > "$run_dir/client.done"
        echo "[$adaptive_id] Warmup failed."
        continue 2
      fi
    done
  fi

  echo "[$adaptive_id] Running measured benchmark with power sampling."
  ...
  if [[ "$scenario" == "capacity_probe" && "$bench_status" -eq 0 ]]; then
    if record_capacity "$benchmark_json" "$model_label" "$dataset" \
      ...
      if [[ "$CAPACITY_PUBLISH" == "1" ]] && \
        ! publish_capacity "$benchmark_json" "$model_label" "$dataset" \
          "$request_rate" "$num_prompts" "$run_id"; then
        echo "Capacity probe could not be published to the shared directory." \
          > "$run_dir/client.failed"
      fi
    else
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "$adaptive_id" "$run_id" "$scenario_id" "$dataset" "$scenario" "$model_label" "$model" \
  ...
  date -Is > "$run_dir/client.done"
  echo "[$adaptive_id] Done."
done

echo "Adaptive client sweep finished. Summary: $SUMMARY_CSV"
