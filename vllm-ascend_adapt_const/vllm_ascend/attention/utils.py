...
class AscendCommonAttentionMetadata(CommonAttentionMetadata):
    ...
    # Whether this metadata represents a non-contiguous compute-aware
    # micro-batch. Such groups must preserve scheduler phase labels when a
    # prefill request is split into a one-token tail.
    compute_aware_grouping: bool = False
    ...
    def unpadded(self, num_actual_tokens: int, num_actual_reqs: int) -> "AscendCommonAttentionMetadata":
        ...
        return AscendCommonAttentionMetadata(
            ...
            compute_aware_grouping=self.compute_aware_grouping,
...
def split_decodes_and_prefills(
    ...
    # Compute-aware grouping may split a chunked-prefill request into a short
    # segment whose query length is at or below the decode threshold. Preserve
    # the scheduler's phase classification when it is available instead of
    # misclassifying that segment as decode from query length alone.
    if (
        common_attn_metadata.compute_aware_grouping
        and common_attn_metadata.is_prefilling is not None
    ):
        is_prefill = common_attn_metadata.is_prefilling[:num_reqs].bool()
        if torch.any(is_prefill):
            first_prefill = is_prefill.int().argmax(dim=-1).item()
            if torch.any(~is_prefill[first_prefill:]):
                raise ValueError(
                    "Attention metadata must keep decode requests before "
                    "prefill requests."
                )
