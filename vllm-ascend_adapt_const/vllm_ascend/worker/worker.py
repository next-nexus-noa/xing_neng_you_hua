...
from vllm_ascend.worker.pp_trace import PPTraceRecorder, pp_trace_enabled
...
class _PPBroadcastSend:
    def __init__(self, handles: list[Handle], tensor_dict: dict[str, torch.Tensor | Any]) -> None:
        self.handles = handles
        self.tensor_dict = tensor_dict


class _PPBroadcastCommunicator:
    """Adjacent PP transfer implemented with broadcast-only process groups."""

    def __init__(self, vllm_config: VllmConfig) -> None:
        pp = get_pp_group()
        self.rank = pp.rank
        self.rank_in_group = pp.rank_in_group
        backend = torch.distributed.get_backend(pp.device_group)
        self._prev_edge: tuple[int, int, Any, Any] | None = None
        self._next_edge: tuple[int, int, Any, Any] | None = None
        ...
        external_dp = world_size // (dp * pp_size * pcp * tp)
        all_ranks = torch.arange(world_size).reshape(
            external_dp, dp, pp_size, pcp, tp
        )
        pp_groups = all_ranks.transpose(2, 4).reshape(-1, pp_size).tolist()
        for pp_ranks in pp_groups:
            for edge_idx in range(pp_size - 1):
                ranks = [pp_ranks[edge_idx], pp_ranks[edge_idx + 1]]
                device_group = torch.distributed.new_group(ranks, backend=backend)
                cpu_group = torch.distributed.new_group(ranks, backend="gloo")
                edge = (ranks[0], ranks[1], device_group, cpu_group)
                if ranks == pp.ranks[self.rank_in_group : self.rank_in_group + 2]:
                    self._next_edge = edge
                if ranks == pp.ranks[self.rank_in_group - 1 : self.rank_in_group + 1]:
                    self._prev_edge = edge

    def _edge_from_prev(self) -> tuple[int, int, Any, Any]:
        assert self._prev_edge is not None
        return self._prev_edge

    def _edge_to_next(self) -> tuple[int, int, Any, Any]:
        assert self._next_edge is not None
        return self._next_edge

    def recv_from_prev(self) -> AsyncIntermediateTensors:
        src, _, device_group, cpu_group = self._edge_from_prev()
        metadata_holder: list[Any] = [None]
        torch.distributed.broadcast_object_list(metadata_holder, src=src, group=cpu_group)
        tensor_dict: dict[str, torch.Tensor | Any] = {}
        handles: list[Handle] = []
        for key, value in metadata_holder[0]:
            if isinstance(value, TensorMetadata):
                ...
                handles.append(
                    torch.distributed.broadcast(tensor, src=src, group=group, async_op=True)
                )
            else:
                tensor_dict[key] = value
        return AsyncIntermediateTensors(tensor_dict, comm_handles=handles)

    def send_to_next(self, tensor_dict: dict[str, torch.Tensor | Any]) -> _PPBroadcastSend:
        src, _, device_group, cpu_group = self._edge_to_next()
        metadata, tensors = _split_tensor_dict(tensor_dict)
        torch.distributed.broadcast_object_list([metadata], src=src, group=cpu_group)
        handles: list[Handle] = []
        ...
        return _PPBroadcastSend(handles, tensor_dict)
...
class NPUWorker(WorkerBase):
    def __init__(
        ...
        **kwargs,
        ...
        self._pp_send_work: list[_PPBroadcastSend] = []
        self._pp_broadcast_comm: _PPBroadcastCommunicator | None = None
        self._pp_trace_recorder: PPTraceRecorder | None = None
    ...
    def init_device(self):
        ...
        self._attach_pp_callbacks_to_model_runner()

    def _attach_pp_callbacks_to_model_runner(self) -> None:
        setattr(self.model_runner, "pp_worker", self)
        setattr(
            self.model_runner,
            "pp_broadcast_intermediate_tensors",
            self._broadcast_intermediate_tensors_from_runner,
        )
        setattr(
            self.model_runner,
            "pp_recv_intermediate_tensors",
            self._recv_intermediate_tensors_from_runner,
        )

    def _get_pp_trace_recorder(self) -> PPTraceRecorder | None:
        if not pp_trace_enabled():
            return None
        if self._pp_trace_recorder is None:
            pp = get_pp_group()
            self._pp_trace_recorder = PPTraceRecorder(
                rank=self.rank,
                pp_rank=pp.rank_in_group,
                pp_size=pp.world_size,
            )
        if self.model_runner is not None:
            setattr(self.model_runner, "pp_trace_recorder", self._pp_trace_recorder)
        return self._pp_trace_recorder

    def _broadcast_intermediate_tensors_from_runner(
        self, tensor_dict: dict[str, torch.Tensor | Any]
    ) -> None:
        if self._pp_broadcast_comm is None:
            self._pp_broadcast_comm = _PPBroadcastCommunicator(self.vllm_config)
        self._pp_send_work.append(
            self._pp_broadcast_comm.send_to_next(tensor_dict)
        )

    def _recv_intermediate_tensors_from_runner(self) -> AsyncIntermediateTensors:
        if self._pp_broadcast_comm is None:
            self._pp_broadcast_comm = _PPBroadcastCommunicator(self.vllm_config)
        return self._pp_broadcast_comm.recv_from_prev()

    def _runner_handles_pp_intermediate_tensors(self) -> bool:
        parallel_config = self.vllm_config.parallel_config
        return (
            envs_ascend.VLLM_ASCEND_PP_MICROBATCH
            or bool(getattr(parallel_config, "enable_adaptive_ubatch", False))
    ...
    def execute_model(
        ...
        trace = self._get_pp_trace_recorder()
        if trace is not None:
            trace.begin_step(scheduler_output.total_num_scheduled_tokens)
        ...
        if self._pp_send_work:
            measure_adaptive_send_wait = bool(
                getattr(
            ...
            send_wait_start = (
                time.perf_counter() if measure_adaptive_send_wait else None
            )
            with trace.stage("worker.wait_prev_pp_send") if trace else nullcontext():
                for send in self._pp_send_work:
                    for handle in send.handles:
                        handle.wait()
                self._pp_send_work = []
            if send_wait_start is not None:
                add_send_wait = getattr(
                    self.model_runner,
                    "_add_pending_adaptive_pp_send_wait",
                    None,
                )
                if add_send_wait is not None:
                    add_send_wait(
        ...
        pp = get_pp_group()
        if forward_pass and pp.world_size > 1 and self._pp_broadcast_comm is None:
            self._pp_broadcast_comm = _PPBroadcastCommunicator(self.vllm_config)
        runner_handles_pp = self._runner_handles_pp_intermediate_tensors()
        if forward_pass and not get_pp_group().is_first_rank and not runner_handles_pp:
            assert self._pp_broadcast_comm is not None
            with trace.stage("worker.recv_intermediate") if trace else nullcontext():
                intermediate_tensors = self._pp_broadcast_comm.recv_from_prev()

        if self.profiler is not None:
            self.profiler.step()

        self._attach_pp_callbacks_to_model_runner()
        with trace.stage("worker.model_runner_execute") if trace else nullcontext():
        ...
        assert self._pp_broadcast_comm is not None
        with trace.stage("worker.send_intermediate") if trace else nullcontext():
            self._pp_send_work = [self._pp_broadcast_comm.send_to_next(output.tensors)]
    ...
    def sample_tokens(self, grammar_output: "GrammarOutput") -> ModelRunnerOutput | AsyncModelRunnerOutput:
        ...
        finish_adaptive_measurement = (
            getattr(
                self.model_runner,
                "_finish_adaptive_pp_step_measurement",
                None,
            )
        ...
        completed = False
        try:
            with trace.stage("worker.sample_tokens") if trace else nullcontext():
                output = self.model_runner.sample_tokens(grammar_output)
            completed = True
            return output
        except Exception as exc:
            if finish_adaptive_measurement is not None:
                finish_adaptive_measurement(
                    failure_reason=type(exc).__name__,
                )
            raise
        finally:
            if completed and finish_adaptive_measurement is not None:
                finish_adaptive_measurement()
            if trace is not None:
                trace.finish_step()
