...
SCOM_PLAN_CACHE_MAX_ENTRIES = 64
...
class NPUModelRunner(GPUModelRunner):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        (
            self.compute_aware_min_tokens,
            self.compute_aware_min_predicted_gain,
            self.compute_aware_quantum,
        ) = self._compute_aware_grouping_config()
        (
            self.scom_min_predicted_gain,
            self.scom_shape_buckets,
            self.scom_optimize_capacities,
            self.scom_allow_bucket_crossing,
            self.scom_capacity_quantum,
            self.scom_max_capacity_candidates,
            self.scom_max_swaps,
        ) = self._scom_grouping_config()
        self._scom_plan_cache: OrderedDict[
            tuple[Any, ...],
            ScomGroupingPlan,
        ] = OrderedDict()
        ...
        # Construct the controller lazily after the PP group is available.
        # Every PP rank mirrors the deterministic controller state, while only
        # the first rank owns the optional decision trace file.
        self.adaptive_ubatch_controller: AdaptiveUBatchController | None = None
        self._adaptive_pp_feedback_step = 0
        self._pending_adaptive_pp_decision: AdaptiveUBatchDecision | None = None
    ...
    def _pp_trace_stage(self, name: str, sync_device: bool = False):
        trace = getattr(self, "pp_trace_recorder", None)
        if trace is None:
            return nullcontext()
        return trace.stage(name, sync_device=sync_device)

    def _pp_microbatch_enabled(self) -> bool:
        return (
            (
                bool(int(os.getenv("VLLM_ASCEND_PP_MICROBATCH", "0")))
                or bool(getattr(self.parallel_config, "enable_adaptive_ubatch", False))
                or int(getattr(self.parallel_config, "ubatch_size", 0) or 0) > 1
            )
            and get_pp_group().world_size > 1
        )

    def _pp_microbatch_configured_count(self) -> int:
        env_value = os.getenv("VLLM_ASCEND_PP_MICROBATCH_NUM")
        if env_value is not None:
            configured = int(env_value)
        elif bool(getattr(self.parallel_config, "enable_adaptive_ubatch", False)):
            configured = int(getattr(self.parallel_config, "adaptive_ubatch_max_size", 4))
        else:
            configured = int(getattr(self.parallel_config, "ubatch_size", 0) or 2)
        return max(1, configured)

    @staticmethod
    def _pp_microbatch_grouping_mode() -> str:
        mode = str(
            ascend_envs.VLLM_ASCEND_PP_MICROBATCH_GROUPING
        ).strip().lower()
        if mode not in {"compute_aware", "scom", "uniform"}:
            raise ValueError(
                "VLLM_ASCEND_PP_MICROBATCH_GROUPING must be "
                "'compute_aware', 'scom', or 'uniform'."
            )
        return mode

    def _compute_aware_grouping_supported(
        self,
        scheduler_output: "SchedulerOutput",
    ) -> tuple[bool, str]:
        if self.speculative_config is not None:
            return False, "speculative_decoding"
        if self.is_pooling_model:
            return False, "pooling_model"
        if self.broadcast_pp_output:
            return False, "broadcast_pp_output"
        if self.pcp_size > 1 or self.dcp_size > 1 or self.use_cp:
            return False, "context_parallel"
        if self.use_compress:
            return False, "compressed_attention"
        if self._has_gdn:
            return False, "gdn_attention"
        if self.cache_config.mamba_cache_mode != "none":
            return False, "mamba_cache"
        if self.cache_config.kv_sharing_fast_prefill:
            return False, "kv_sharing_fast_prefill"
        if scheduler_output.scheduled_encoder_inputs:
            return False, "encoder_inputs"
        return True, "supported"

    @staticmethod
    def _compute_aware_grouping_config() -> tuple[int, float, int]:
        min_tokens = int(
            ascend_envs.VLLM_ASCEND_PP_COMPUTE_AWARE_MIN_TOKENS
        )
        min_gain_pct = int(
            ascend_envs.VLLM_ASCEND_PP_COMPUTE_AWARE_MIN_GAIN_PCT
        )
        quantum = int(
            ascend_envs.VLLM_ASCEND_PP_COMPUTE_AWARE_QUANTUM
        )
        if min_tokens < 0:
            raise ValueError(
                "VLLM_ASCEND_PP_COMPUTE_AWARE_MIN_TOKENS must be "
                "non-negative."
            )
        if not 0 <= min_gain_pct <= 100:
            raise ValueError(
                "VLLM_ASCEND_PP_COMPUTE_AWARE_MIN_GAIN_PCT must be "
                "between 0 and 100."
            )
        if quantum < 1:
            raise ValueError(
                "VLLM_ASCEND_PP_COMPUTE_AWARE_QUANTUM must be positive."
            )
        return min_tokens, min_gain_pct / 100.0, quantum

    @staticmethod
    def _scom_grouping_config() -> tuple[
        float,
        tuple[int, ...],
        bool,
        bool,
        ...
        min_gain_pct = int(
            ascend_envs.VLLM_ASCEND_PP_SCOM_MIN_GAIN_PCT
        )
        shape_buckets = parse_shape_buckets(
            ascend_envs.VLLM_ASCEND_PP_SCOM_SHAPE_BUCKETS
        )
        optimize_capacities = bool(
            ascend_envs.VLLM_ASCEND_PP_SCOM_OPTIMIZE_CAPACITIES
        )
        allow_bucket_crossing = bool(
            ascend_envs.VLLM_ASCEND_PP_SCOM_ALLOW_BUCKET_CROSSING
        )
        capacity_quantum = int(
            ascend_envs.VLLM_ASCEND_PP_SCOM_CAPACITY_QUANTUM
        )
        max_capacity_candidates = int(
            ascend_envs.VLLM_ASCEND_PP_SCOM_MAX_CAPACITY_CANDIDATES
        )
        max_swaps = int(
            ascend_envs.VLLM_ASCEND_PP_SCOM_MAX_SWAPS
        )
        if not 0 <= min_gain_pct <= 100:
            raise ValueError(
                "VLLM_ASCEND_PP_SCOM_MIN_GAIN_PCT must be "
                "between 0 and 100."
            )
        if max_capacity_candidates < 1:
            raise ValueError(
                "VLLM_ASCEND_PP_SCOM_MAX_CAPACITY_CANDIDATES "
                "must be positive."
            )
        if capacity_quantum < 1:
            raise ValueError(
                "VLLM_ASCEND_PP_SCOM_CAPACITY_QUANTUM must be "
                "positive."
            )
        if max_swaps < 0:
            raise ValueError(
                "VLLM_ASCEND_PP_SCOM_MAX_SWAPS must be "
                "non-negative."
            )
        return (
            min_gain_pct / 100.0,
            shape_buckets,
            optimize_capacities,
            allow_bucket_crossing,
            capacity_quantum,
            max_capacity_candidates,
            max_swaps,
        )

    def _select_compute_aware_grouping(
        self,
        *,
        ...
        # SchedulerOutput and InputBatch request order are identical on every
        # PP rank. Compute the deterministic grouping locally instead of
        # broadcasting a Python payload in the per-step critical path.
        return plan_compute_aware_groups(
    ...
    def _select_scom_grouping(
        ...
        *,
        ...
        scheduled = tuple(
            int(value)
            for value in num_scheduled_tokens_np[:num_reqs]
        )
        computed = tuple(
            int(value)
            for value in (
        ...
        cached_plan = self._scom_plan_cache.get(cache_key)
        if cached_plan is not None:
            self._scom_plan_cache.move_to_end(cache_key)
            return replace(
                cached_plan,
                cache_hit=True,
        ...
        local_plan = plan_scom_groups(
            num_scheduled_tokens=scheduled,
            num_computed_tokens=computed,
            num_ubatches=num_ubatches,
            quantum=self.compute_aware_quantum,
            capacity_quantum=self.scom_capacity_quantum,
            min_predicted_gain=self.scom_min_predicted_gain,
            shape_buckets=self.scom_shape_buckets,
            optimize_capacities=self.scom_optimize_capacities,
            allow_bucket_crossing=self.scom_allow_bucket_crossing,
            max_capacity_candidates=self.scom_max_capacity_candidates,
            max_swaps=self.scom_max_swaps,
        )
        self._scom_plan_cache[cache_key] = local_plan
        if len(self._scom_plan_cache) > SCOM_PLAN_CACHE_MAX_ENTRIES:
            self._scom_plan_cache.popitem(last=False)
        return local_plan

    def _pp_microbatch_count(
        self,
        num_tokens: int,
        ...
        if bool(getattr(self.parallel_config, "enable_adaptive_ubatch", False)):
            if num_scheduled_tokens_np is None:
                return 1
            return self._select_adaptive_pp_microbatch_count(
                num_scheduled_tokens_np
            ).num_ubatches
        return max(1, min(self._pp_microbatch_configured_count(), num_tokens))
    ...
    def _select_adaptive_pp_microbatch_count(
        ...
        if self.adaptive_ubatch_controller is None:
            self.adaptive_ubatch_controller = AdaptiveUBatchController(
                ...
                trace_enabled=get_pp_group().is_first_rank,
            )
        ...
        if num_ubatches != decision.num_ubatches:
            decision = self._commit_adaptive_pp_effective_m(
                decision,
                effective_m=num_ubatches,
                reason=f"capped_to_{max_count}",
            )
        return decision

    def _commit_adaptive_pp_effective_m(
        self,
        ...
        *,
        ...
        if self.adaptive_ubatch_controller is not None:
            commit = getattr(
                self.adaptive_ubatch_controller,
                "commit_effective_m",
                None,
            )
    ...
    def _use_mirrored_adaptive_pp_microbatch_decision(
        ...
        """Return a decision produced independently on every PP rank.

        All PP ranks receive the same SchedulerOutput, run the same
        deterministic selector, and apply the same MAX-reduced observations.
        Their controller states therefore remain mirrored without adding a
        blocking CPU collective to every scheduler step.

        Cross-rank timing feedback remains synchronized at the configured
        sampling interval. Only the first rank writes decision traces, so
        mirrored controllers do not duplicate or corrupt trace records.
        """
        return decision

    def _observe_adaptive_pp_microbatch_result(
        self,
    ...
    def _flush_adaptive_pp_microbatch_result(
        ...
        Feedback is reduced at the next decision point, where all PP ranks are
        already aligned. Reducing immediately after forward can deadlock for
        M=1 because downstream ranks may still be waiting for the normal PP
        intermediate-tensor send.
        """
        decision = self._pending_adaptive_pp_decision
        ...
        if self.adaptive_ubatch_controller is not None:
            logger.info_once(
                "Adaptive PP feedback scope: full worker step "
                "(forward + sampling + post-processing)."
            )

    def _pending_adaptive_pp_local_elapsed(self) -> float:
        """Resolve the previous stage-local full-step critical-path time."""
        elapsed_ms = max(
            0.0,
            float(
                getattr(
                    self,
                    "_pending_adaptive_pp_elapsed_ms",
                    0.0,
                )
                or 0.0
            ),
        ...
        npu_events = getattr(
            self,
            "_pending_adaptive_pp_npu_events",
            None,
        )
        if npu_events is not None:
            start_event, end_event = npu_events
            # Worker waits the previous normal PP send before launching the
            # feedback reduction, so reading this event pair cannot disturb
            # the pipeline communication order.
            try:
                npu_elapsed_ms = float(start_event.elapsed_time(end_event))
                if math.isfinite(npu_elapsed_ms) and npu_elapsed_ms >= 0.0:
                    elapsed_ms = max(elapsed_ms, npu_elapsed_ms)
            except (RuntimeError, TypeError, ValueError):
                logger.debug(
                    "Unable to read adaptive PP NPU event timing; "
                    "falling back to CPU full-step timing.",
                    exc_info=True,
                )
        return elapsed_ms + max(
            ...
            float(
                getattr(
                    self,
                    "_pending_adaptive_pp_send_wait_ms",
                    0.0,
                )
            ),
        )
    ...
    def _should_measure_adaptive_pp_critical_path(
        ...
        # There is nothing to learn when M=1 is the only executable
        # candidate. Skipping measurement also avoids an unnecessary periodic
        ...
        if candidate_scores and len(candidate_scores) <= 1:
            return False
        self._adaptive_pp_feedback_step += 1
        interval = max(
            1,
            int(
                getattr(
                    self.parallel_config,
                    "adaptive_ubatch_feedback_interval_steps",
                    1,
                )
            ),
        )
    ...
    def _queue_adaptive_pp_microbatch_result(
        self,
        decision: AdaptiveUBatchDecision,
        *,
        elapsed_ms: float,
    ...
    def _add_pending_adaptive_pp_send_wait(self, elapsed_ms: float) -> None:
        ...
        pending_ms = getattr(self, "_pending_adaptive_pp_elapsed_ms", None)
        if pending_ms is None:
            return
        self._pending_adaptive_pp_send_wait_ms = (
            max(
                0.0,
                float(
                    getattr(
    ...
    def _finish_adaptive_pp_step_measurement(
        self,
        *,
        failure_reason: str | None = None,
    ) -> None:
        """Finish trace-independent feedback after sampling/post-processing.

        Timing only the model forward made M>1 look attractive while omitting
        sampling, post-processing, and PP communication tails that determine
        end-to-end latency. Both PP ranks finish here; the next decision
        reduces their elapsed times with MAX as before.
        """
        # Platform-specific subclasses such as NPUModelRunner310 can inherit
        # this method without executing NPUModelRunner.__init__. Missing state
        # therefore means that no measurement is active for this step.
        decision = getattr(self, "_active_adaptive_pp_decision", None)
        start = getattr(self, "_active_adaptive_pp_step_start", None)
        npu_start_event = getattr(
            self,
            "_active_adaptive_pp_npu_start_event",
            None,
        )
        self._active_adaptive_pp_decision = None
        self._active_adaptive_pp_step_start = None
        ...
        if npu_start_event is not None:
            try:
                npu_end_event = torch.npu.Event(enable_timing=True)
                npu_end_event.record()
                npu_events = (npu_start_event, npu_end_event)
            except (RuntimeError, TypeError):
                logger.debug(
                    "Unable to record adaptive PP end event; "
                    "falling back to CPU full-step timing.",
                    exc_info=True,
                )
    ...
    def _observe_adaptive_pp_microbatch_failure(
        self,
        decision: AdaptiveUBatchDecision | None,
        *,
        ...
        if self.adaptive_ubatch_controller is None:
            return
        self.adaptive_ubatch_controller.observe_failure(
            decision,
            reason=reason,
        )

    def _observe_adaptive_pp_microbatch_rejection(
        self,
        decision: AdaptiveUBatchDecision | None,
        *,
        reason: str,
        ...
        if observe_rejection is not None:
            observe_rejection(decision, reason=reason)

    def _metadata_builder_count(self) -> int:
        num_builders = (
        ...
        if (
            ...
            num_builders = max(num_builders, self._pp_microbatch_configured_count())
        return max(1, num_builders)

    def _ensure_pp_microbatch_callbacks(self) -> None:
        worker = getattr(self, "pp_worker", None)
        if worker is None:
            return
        if getattr(self, "pp_broadcast_intermediate_tensors", None) is None:
            send = getattr(worker, "_broadcast_intermediate_tensors_from_runner", None)
            if send is not None:
                setattr(self, "pp_broadcast_intermediate_tensors", send)
        if getattr(self, "pp_recv_intermediate_tensors", None) is None:
            recv = getattr(worker, "_recv_intermediate_tensors_from_runner", None)
            if recv is not None:
                setattr(self, "pp_recv_intermediate_tensors", recv)

    def _validate_pp_microbatch_supported(self) -> None:
        if self.speculative_config is not None:
            raise RuntimeError("PP microbatch currently does not support speculative decoding.")
        if self.is_pooling_model:
            raise RuntimeError("PP microbatch currently does not support pooling models.")
        if self.broadcast_pp_output:
            raise RuntimeError("PP microbatch currently does not support broadcast_pp_output.")
        if self.pcp_size > 1 or self.dcp_size > 1 or self.use_cp:
            raise RuntimeError("PP microbatch currently does not support PCP/DCP/CP.")
        if self.use_aux_hidden_state_outputs:
            raise RuntimeError("PP microbatch currently does not support aux hidden states.")
        if enable_sp(self.vllm_config) or enable_sp_by_pass():
            raise RuntimeError("PP microbatch currently does not support SP/FlashComm1.")
    ...
    def _slice_optional_tensor(
        ...
        return tensor[token_slice] if tensor is not None else None
    ...
    def _index_optional_tensor(
        ...
        return tensor.index_select(
            0,
            token_indices.to(device=tensor.device),
        )
    ...
    def _index_positions(
        ...
        indices = token_indices.to(device=positions.device)
        if positions.dim() == 1:
            return positions.index_select(0, indices)
        return positions.index_select(1, indices)

    @staticmethod
    def _index_intermediate_tensors(
        ...
        return IntermediateTensors(
            {
                name: value.index_select(
                    0,
                    token_indices.to(device=value.device),
                )
    ...
    def _run_pp_microbatch_forward(
        self,
        *,
        ...
        compute_aware_plan: (
            ComputeAwareGroupingPlan | ScomGroupingPlan | None
        ),
        ...
        pp = get_pp_group()
        self._ensure_pp_microbatch_callbacks()
        send_intermediate = getattr(self, "pp_broadcast_intermediate_tensors", None)
        recv_intermediate = getattr(self, "pp_recv_intermediate_tensors", None)
        if send_intermediate is not None and not callable(send_intermediate):
            raise RuntimeError("pp_broadcast_intermediate_tensors must be callable.")
        if recv_intermediate is not None and not callable(recv_intermediate):
            raise RuntimeError("pp_recv_intermediate_tensors must be callable.")
        if not pp.is_last_rank and send_intermediate is None:
            raise RuntimeError("PP microbatch requires pp_broadcast_intermediate_tensors.")
        if not pp.is_first_rank and recv_intermediate is None:
            raise RuntimeError("PP microbatch requires pp_recv_intermediate_tensors.")
        if not isinstance(attn_metadata, list):
            raise RuntimeError("PP microbatch requires sliced attention metadata.")

        if compute_aware_plan is not None:
            work_items: Sequence[
                UBatchSlice | ComputeAwareMicroBatch
            ] = compute_aware_plan.groups
        else:
            if ubatch_slices is None:
                raise RuntimeError(
                    "PP microbatch requires slices or a grouping plan."
                )
            work_items = ubatch_slices

        outputs: list[torch.Tensor] = []
        for ubid, ubatch in enumerate(work_items):
            if isinstance(ubatch, ComputeAwareMicroBatch):
                token_indices = torch.tensor(
                    ubatch.token_indices,
                    dtype=torch.long,
                    device=positions.device,
                )
                token_slice = None
                mb_num_tokens = ubatch.num_tokens
                mb_num_reqs = ubatch.num_requests
            else:
                token_indices = None
                token_slice = ubatch.token_slice
                mb_num_tokens = ubatch.num_tokens
                mb_num_reqs = (
                    ubatch.request_slice.stop
                    - ubatch.request_slice.start
                )
            with self._pp_trace_stage(f"runner.recv.microbatch.{ubid}"):
                mb_intermediate_tensors = (
                    recv_intermediate() if not pp.is_first_rank else None
                )
            if pp.is_first_rank and intermediate_tensors is not None:
                if token_indices is None:
                    assert token_slice is not None
                    mb_intermediate_tensors = intermediate_tensors[
                        token_slice
                    ]
                else:
                    mb_intermediate_tensors = (
                        self._index_intermediate_tensors(
            ...
            if token_indices is None:
                assert token_slice is not None
                mb_input_ids = self._slice_optional_tensor(
                    input_ids,
                    token_slice,
                )
                mb_positions = self._slice_positions(
                    positions,
                    token_slice,
                )
                mb_inputs_embeds = self._slice_optional_tensor(
                    inputs_embeds,
                    token_slice,
                )
            else:
                mb_input_ids = self._index_optional_tensor(
                    input_ids,
                    token_indices,
                )
                mb_positions = self._index_positions(
                    positions,
                    token_indices,
                )
                mb_inputs_embeds = self._index_optional_tensor(
                    inputs_embeds,
                    token_indices,
                )
            if mb_positions.dim() == 1:
                update_cos_sin(mb_positions)
            mb_batch_desc = BatchDescriptor(
                num_tokens=mb_num_tokens,
                num_reqs=mb_num_reqs,
            )
            with (
                self._pp_trace_stage(f"runner.forward.microbatch.{ubid}", sync_device=True),
                set_ascend_forward_context(
                    attn_metadata[ubid],
                    self.vllm_config,
                    num_tokens=mb_num_tokens,
                    ...
                    batch_descriptor=mb_batch_desc,
                    num_actual_tokens=mb_num_tokens,
            ...
            if not pp.is_last_rank:
                assert isinstance(mb_output, IntermediateTensors)
                mb_output.kv_connector_output = kv_connector_output
                self.kv_connector_output = kv_connector_output
                with self._pp_trace_stage(f"runner.send.microbatch.{ubid}"):
                    send_intermediate(mb_output.tensors)
                continue

            assert isinstance(mb_output, torch.Tensor)
        ...
        if compute_aware_plan is not None:
            inverse = torch.tensor(
                compute_aware_plan.inverse_permutation,
                dtype=torch.long,
                device=hidden_states.device,
            )
            hidden_states = hidden_states.index_select(0, inverse)
    ...
    def execute_model(
        ...
        with (
            ...
            self._pp_trace_stage("runner.prepare_input"),
        ):
            with self.synchronize_input_prep():
                ...
                pp_microbatch_enabled = self._pp_microbatch_enabled()
                pp_microbatch_count = 1
                adaptive_pp_decision: AdaptiveUBatchDecision | None = None
                compute_aware_plan: (
                    ComputeAwareGroupingPlan
                    | ScomGroupingPlan
                    | None
                ) = None
                adaptive_pp_measure = False
                ...
                if pp_microbatch_enabled:
                    if bool(getattr(self.parallel_config, "enable_adaptive_ubatch", False)):
                        ...
                        adaptive_pp_decision = (
                            self._use_mirrored_adaptive_pp_microbatch_decision(
                                local_decision
                            )
                        )
                        adaptive_pp_measure = (
                            self._should_measure_adaptive_pp_critical_path(
                                adaptive_pp_decision
                            )
                        )
                        effective_count = min(
                            adaptive_pp_decision.num_ubatches,
                            max(1, num_tokens_unpadded),
                        )
                        if effective_count != adaptive_pp_decision.num_ubatches:
                            adaptive_pp_decision = (
                                self._commit_adaptive_pp_effective_m(
                                    adaptive_pp_decision,
                                    effective_m=effective_count,
                                    reason="capped_to_num_tokens",
                                )
                            )
                        pp_microbatch_count = effective_count
                        logger.debug(
                            "Adaptive PP mirrored microbatch decision: "
                            "pp_rank=%s/%s M=%s gain=%.2f%% reason=%s "
                            "online=%s bucket=%s total_tokens=%s num_reqs=%s",
                            get_pp_group().rank_in_group,
                            get_pp_group().world_size,
                            pp_microbatch_count,
                            adaptive_pp_decision.predicted_gain_pct,
                            adaptive_pp_decision.reason,
                            adaptive_pp_decision.online,
                            adaptive_pp_decision.bucket_key,
                            adaptive_pp_decision.total_tokens
                            or num_tokens_unpadded,
                            adaptive_pp_decision.num_reqs or num_reqs,
                        )
                        trace = getattr(self, "pp_trace_recorder", None)
                        if trace is not None:
                            trace.update_step_meta(
                                adaptive_ubatch_decision=(
                                    adaptive_pp_decision.to_payload()),
                                adaptive_ubatch_effective_m=int(
                                    pp_microbatch_count),
                                adaptive_ubatch_active=bool(
                                    pp_microbatch_count > 1),
                            )
                    else:
                ...
                if pp_microbatch_active:
                    try:
                        self._validate_pp_microbatch_supported()
                    except RuntimeError as exc:
                        if adaptive_pp_decision is None:
                            raise
                        self._observe_adaptive_pp_microbatch_rejection(
                            adaptive_pp_decision,
                            reason=f"unsupported: {exc}",
                        )
                        logger.warning(
                            "Adaptive PP microbatch M=%s is not supported for "
                            "this batch/configuration, falling back to M=1: %s",
                            pp_microbatch_count,
                            exc,
                        )
                        pp_microbatch_count = 1
                        pp_microbatch_active = False
                        adaptive_pp_decision = (
                            self._commit_adaptive_pp_effective_m(
                                adaptive_pp_decision,
                                effective_m=1,
                                reason="unsupported_fallback_m1",
                                fallback=True,
                            )
                        )
                        trace = getattr(self, "pp_trace_recorder", None)
                        if trace is not None:
                            trace.update_step_meta(
                                adaptive_ubatch_decision=(
                                    adaptive_pp_decision.to_payload()),
                                adaptive_ubatch_effective_m=1,
                                adaptive_ubatch_active=False,
                            )
                        should_ubatch = False
                    else:
                        ...
                        should_ubatch = True
                elif pp_microbatch_enabled:
                    should_ubatch = False
                grouping_mode = self._pp_microbatch_grouping_mode()
                if (
                    should_ubatch
                    and pp_microbatch_active
                    and grouping_mode in {"compute_aware", "scom"}
                ):
                    grouping_supported, grouping_reason = (
                        self._compute_aware_grouping_supported(
                            scheduler_output
                        )
                    )
                    if grouping_supported:
                        if not should_plan_compute_aware_groups(
                            total_tokens=num_tokens_unpadded,
                            min_tokens=self.compute_aware_min_tokens,
                        ):
                            grouping_trace = {
                                "applied": False,
                            ...
                            logger.debug(
                                "%s PP grouping skipped: "
                                "total_tokens=%s min_tokens=%s.",
                                grouping_mode,
                                num_tokens_unpadded,
                                self.compute_aware_min_tokens,
                            )
                        else:
                            with self._pp_trace_stage(
                                f"runner.{grouping_mode}_grouping"
                            ):
                                if grouping_mode == "scom":
                                    selected_plan = (
                                        self._select_scom_grouping(
                                            num_scheduled_tokens_np=(
                                                num_scheduled_tokens_np
                                            ),
                                            num_reqs=num_reqs,
                                    ...
                                    selected_plan = (
                                        self._select_compute_aware_grouping(
                                            num_scheduled_tokens_np=(
                                                num_scheduled_tokens_np
                                            ),
                                            num_reqs=num_reqs,
                            ...
                            if selected_plan.applied:
                                compute_aware_plan = selected_plan
                            if grouping_mode == "scom":
                                logger.debug(
                                    "SCOM PP grouping: applied=%s "
                                    "M=%s gain=%.2f%% reason=%s "
                                    "decision_us=%.1f capacities=%s "
                                    "objective=%.3f->%.3f "
                                    "capacity_candidates=%s swaps=%s "
                                    "source=%s cache_hit=%s",
                                    selected_plan.applied,
                                    selected_plan.num_ubatches,
                                    (
                                        selected_plan.predicted_gain
                                        * 100.0
                                    ),
                                    selected_plan.reason,
                                    selected_plan.decision_overhead_us,
                                    selected_plan.selected_capacities,
                                    selected_plan.baseline_objective,
                                    selected_plan.selected_objective,
                                    (
                                        selected_plan
                                        .capacity_candidate_count
                                    ),
                                    selected_plan.mapping_swap_count,
                                    selected_plan.selection_source,
                                    selected_plan.cache_hit,
                                )
                            else:
                                logger.debug(
                                    "Compute-aware PP grouping: "
                                    "applied=%s M=%s gain=%.2f%% "
                                    "reason=%s decision_us=%.1f "
                                    "uniform_costs=%s "
                                    "candidate_costs=%s",
                                    selected_plan.applied,
                                    selected_plan.num_ubatches,
                                    ...
                                    selected_plan.reason,
                                    selected_plan.decision_overhead_us,
                                    selected_plan.uniform_group_costs,
                                    selected_plan.candidate_group_costs,
                                )
                            grouping_trace = (
                                selected_plan.to_payload()
                            )
                            if not selected_plan.applied:
                                grouping_trace.pop("groups", None)
                        trace = getattr(
                            self,
                            "pp_trace_recorder",
                            None,
                        ...
                        if trace is not None:
                            grouping_trace.pop("permutation", None)
                            grouping_trace.pop(
                                "inverse_permutation",
                                None,
                            )
                        ...
                        logger.debug(
                            "%s PP grouping unsupported for "
                            "this step (%s); using contiguous uniform "
                            "micro-batches.",
                            grouping_mode,
                            grouping_reason,
                        )
            ...
            with self._pp_trace_stage("runner.preprocess"):
                preprocess_intermediate_tensors = intermediate_tensors
                restore_sync_and_slice = None
                runner_receives_pp_intermediate = (
                    pp_microbatch_enabled
                    and not get_pp_group().is_first_rank
                    and intermediate_tensors is None
                )
                use_pp_microbatch_placeholder = (
                    runner_receives_pp_intermediate and pp_microbatch_active
                )
                if use_pp_microbatch_placeholder:
                    ...
                    restore_sync_and_slice = self.sync_and_slice_intermediate_tensors

                    def _defer_pp_microbatch_intermediate_sync(
                        num_tokens: int,
                        placeholder: IntermediateTensors | None,
                        sync_self: bool,
                    ) -> IntermediateTensors:
                        assert placeholder is not None
                        return placeholder

                    self.sync_and_slice_intermediate_tensors = (  # type: ignore[method-assign]
                        _defer_pp_microbatch_intermediate_sync
                    )
                elif runner_receives_pp_intermediate:
                    self._ensure_pp_microbatch_callbacks()
                    recv_intermediate = getattr(
                        self, "pp_recv_intermediate_tensors", None
                    )
                    if recv_intermediate is None or not callable(recv_intermediate):
                        raise RuntimeError(
                            "PP runner-owned receive requires "
                            "pp_recv_intermediate_tensors."
                        )
                    with self._pp_trace_stage("runner.recv.full_batch"):
                        preprocess_intermediate_tensors = recv_intermediate()
                try:
                    (
                        input_ids,
                        inputs_embeds,
                    ...
                    if restore_sync_and_slice is not None:
                        self.sync_and_slice_intermediate_tensors = restore_sync_and_slice  # type: ignore[method-assign]
                if use_pp_microbatch_placeholder:
                    intermediate_tensors = None

            # update global cos, sin
            if not pp_microbatch_active:
        ...
        try:
            with self.maybe_get_kv_connector_output(
                ...
                **(
                ...
                if pp_microbatch_active:
                    ...
                    hidden_states = self._run_pp_microbatch_forward(
                        input_ids=input_ids,
                        positions=positions,
                        ...
                        compute_aware_plan=compute_aware_plan,
                        num_tokens_across_dp=num_tokens_across_dp,
                    ...
                    with (
                        ...
                        self._pp_trace_stage("runner.forward", sync_device=True),
                        set_ascend_forward_context(
                            attn_metadata,
                            self.vllm_config,
                            num_tokens=num_tokens_padded,
            ...
            if adaptive_observe:
                self._finish_adaptive_pp_step_measurement(
                    failure_reason=type(exc).__name__,
                )
            raise
        with (
            record_function_or_nullcontext("post process"),
            self._pp_trace_stage("runner.post_process", sync_device=True),
        ):
            if pp_microbatch_active and not get_pp_group().is_last_rank:
                self._finalize_dump_data()
                return None
            ...
            if not self.broadcast_pp_output:
                ...
                if not get_pp_group().is_last_rank:
                    sample_hidden_states = hidden_states[logits_indices]
                    send_pp_intermediate_tensors = getattr(
                        self, "pp_broadcast_intermediate_tensors", None
                    )
                    if send_pp_intermediate_tensors is None:
                        raise RuntimeError(
                            "PP broadcast output requires the worker to install "
                            "pp_broadcast_intermediate_tensors."
                        )
                    send_pp_intermediate_tensors(hidden_states.tensors)
                    logits = None
                else:
                    sample_hidden_states = hidden_states[logits_indices]
                    logits = self.model.compute_logits(sample_hidden_states)
    ...
    def sample_tokens(
        ...
        with self._pp_trace_stage("runner.sample", sync_device=True):
            # Apply structured output bitmasks if present.
        ...
        with self._pp_trace_stage("runner.bookkeeping"):
            (
                logprobs_lists,
                valid_sampled_token_ids,
        ...
        with (
            ...
            self._pp_trace_stage("runner.draft_token", sync_device=True),
        ):
            if self.speculative_config:
        ...
        if self.need_accepted_tokens:
            assert self.sampling_done_event is not None
            with self._pp_trace_stage("runner.async_state_update", sync_device=True):
                with (
    ...
    def _pp_broadcast_prev_sampled_token_ids(
        ...
        """Broadcast sampled token ids from the last PP stage."""
        pp = get_pp_group()
    ...
    def _pp_receive_prev_sampled_token_ids_to_input_batch(self) -> None:
        """Receive sampled token ids broadcast from the last PP stage."""
        pp = get_pp_group()
    ...
    def _build_attention_metadata(
        ...
        compute_aware_groups: (
            tuple[ComputeAwareMicroBatch, ...] | None
        ) = None,
        ...
        attn_metadata: PerLayerAttnMetadata = {}
        if compute_aware_groups is not None:
            attn_metadata = [
                dict() for _ in range(len(compute_aware_groups))
            ]
        ...
        def _group_ascend_attn_metadata(
            group: ComputeAwareMicroBatch,
            common_attn_metadata: AscendCommonAttentionMetadata,
        ) -> AscendCommonAttentionMetadata:
            if num_scheduled_tokens_np is None:
                raise RuntimeError(
                    "Compute-aware grouping requires scheduled-token "
                    "metadata."
                )
            request_indices_list = [
                segment.request_index for segment in group.segments
            ]
            if len(request_indices_list) != len(
                set(request_indices_list)
            ):
                raise RuntimeError(
                    "A compute-aware micro-batch may contain at most "
                    "one segment per request."
                )
            segment_lengths = [
                segment.num_tokens for segment in group.segments
            ]
            segment_starts = [
            ...
            segment_tails = [
                int(num_scheduled_tokens_np[segment.request_index])
                - segment.request_token_stop
                for segment in group.segments
            ]
            token_indices_list = list(group.token_indices)
            ...
            request_indices_by_device: dict[
                torch.device,
                torch.Tensor,
            ] = {}
            token_indices_by_device: dict[
                torch.device,
                torch.Tensor,
            ] = {}
            tails_by_type: dict[
            ...
            def _index_reqs(value):
                if value is None:
                    return None
                indices = request_indices_by_device.get(
                    value.device
                )
                if indices is None:
                    indices = request_indices_cpu.to(
                        device=value.device
                    )
                    request_indices_by_device[value.device] = indices
                return value.index_select(
                    0,
                    indices,
                )

            def _index_tokens(value):
                if value is None:
                    return None
                indices = token_indices_by_device.get(value.device)
                if indices is None:
                    indices = token_indices_cpu.to(
                        device=value.device
                    )
                    token_indices_by_device[value.device] = indices
                return value.index_select(
                    0,
                    indices,
                )

            def _adjust_seq_lens(value):
                selected = _index_reqs(value)
                if selected is None:
                    return None
                key = (selected.device, selected.dtype)
                tail = tails_by_type.get(key)
                if tail is None:
                    tail = torch.tensor(
                        segment_tails,
                    ...
                    tails_by_type[key] = tail
                return selected.clone() - tail

            def _adjust_computed(value):
                selected = _index_reqs(value)
                if selected is None:
                    return None
                key = (selected.device, selected.dtype)
                starts = starts_by_type.get(key)
                if starts is None:
                    starts = torch.tensor(
                        segment_starts,
                        dtype=selected.dtype,
                    ...
                    starts_by_type[key] = starts
                return selected.clone() + starts

            seq_lens = _adjust_seq_lens(
                common_attn_metadata.seq_lens
            )
            seq_lens_cpu = _adjust_seq_lens(
                common_attn_metadata.seq_lens_cpu
            )
            seq_lens_cpu_upper_bound = _adjust_seq_lens(
                common_attn_metadata.seq_lens_cpu_upper_bound
            )
            raw_seq_lens_cpu = _adjust_seq_lens(
                common_attn_metadata._seq_lens_cpu
            )
            num_computed_tokens_cpu = _adjust_computed(
                common_attn_metadata.num_computed_tokens_cpu
            )
            raw_num_computed_tokens_cpu = _adjust_computed(
                common_attn_metadata._num_computed_tokens_cpu
            )
            max_seq_len_ubatch = (
                int(seq_lens_cpu_upper_bound.max().item())
            ...
            return AscendCommonAttentionMetadata(
                ...
                seq_lens_cpu=seq_lens_cpu,
                num_computed_tokens_cpu=num_computed_tokens_cpu,
                num_reqs=group.num_requests,
                num_actual_tokens=group.num_tokens,
                max_query_len=max(segment_lengths),
                max_seq_len=max_seq_len_ubatch,
                block_table_tensor=_index_reqs(
                    common_attn_metadata.block_table_tensor
                ),
                slot_mapping=_index_tokens(
                ...
                _seq_lens_cpu=raw_seq_lens_cpu,
                _num_computed_tokens_cpu=(
                    raw_num_computed_tokens_cpu
                ),
                dcp_local_seq_lens=_index_reqs(
                    common_attn_metadata.dcp_local_seq_lens
                ),
                dcp_local_seq_lens_cpu=_index_reqs(
                    common_attn_metadata.dcp_local_seq_lens_cpu
                ),
                is_prefilling=_index_reqs(
                    common_attn_metadata.is_prefilling
                ),
                encoder_seq_lens=_index_reqs(
                    common_attn_metadata.encoder_seq_lens
                ),
                encoder_seq_lens_cpu=_index_reqs(
        ...
        def _split_ascend_attn_metadata(
            ...
            results: list[AscendCommonAttentionMetadata] = []

            def _slice_reqs(value, request_slice: slice):
                return value[request_slice] if value is not None else None

            def _slice_tokens(value, token_slice: slice):
                return value[token_slice] if value is not None else None

            for ubatch_slice in slices:
                ...
                seq_lens_cpu = _slice_reqs(
                    common_attn_metadata.seq_lens_cpu, request_slice)
                seq_lens_cpu_upper_bound = _slice_reqs(
                    common_attn_metadata.seq_lens_cpu_upper_bound,
                    request_slice)
                num_computed_tokens_cpu = _slice_reqs(
                    common_attn_metadata.num_computed_tokens_cpu,
                    request_slice)
                _seq_lens_cpu = _slice_reqs(
                    common_attn_metadata._seq_lens_cpu, request_slice)
                ...
                if splits_last_request:
                    ...
                    if _seq_lens_cpu is not None:
                        _seq_lens_cpu = _seq_lens_cpu.clone()
                        _seq_lens_cpu[-1] -= tokens_skipped

                if seq_lens_cpu_upper_bound is not None:
                    max_seq_len_ubatch = int(seq_lens_cpu_upper_bound.max())
                else:
                    max_seq_len_ubatch = common_attn_metadata.max_seq_len
                max_query_len_ubatch = int(torch.max(torch.abs(
                    query_start_loc_cpu[1:] - query_start_loc_cpu[:-1])).item())
                if max_query_len_ubatch == 0:
                    max_query_len_ubatch = common_attn_metadata.max_query_len

                results.append(AscendCommonAttentionMetadata(
                    query_start_loc=query_start_loc,
                    query_start_loc_cpu=query_start_loc_cpu,
                    seq_lens=seq_lens,
                    seq_lens_cpu=seq_lens_cpu,
                    ...
                    max_query_len=max_query_len_ubatch,
                    max_seq_len=max_seq_len_ubatch,
                    ...
                    actual_seq_lengths_q=common_attn_metadata.actual_seq_lengths_q[
                        token_slice],
                    positions=_slice_tokens(common_attn_metadata.positions,
                                            token_slice),
                    positions_cpu=_slice_tokens(common_attn_metadata.positions_cpu,
                                                token_slice),
                    attn_state=common_attn_metadata.attn_state,
                    graph_pad_size=common_attn_metadata.graph_pad_size,
                    ...
                    dcp_local_seq_lens=_slice_reqs(
                        common_attn_metadata.dcp_local_seq_lens,
                        request_slice),
                    dcp_local_seq_lens_cpu=_slice_reqs(
                        common_attn_metadata.dcp_local_seq_lens_cpu,
                        request_slice),
                    is_prefilling=_slice_reqs(
                        common_attn_metadata.is_prefilling,
                        request_slice),
                    encoder_seq_lens=_slice_reqs(
                        common_attn_metadata.encoder_seq_lens,
                        request_slice),
                    encoder_seq_lens_cpu=_slice_reqs(
                        common_attn_metadata.encoder_seq_lens_cpu,
                        request_slice),
        ...
        for kv_cache_gid, kv_cache_group in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            for attn_gid in range(len(self.attn_groups[kv_cache_gid])):
                if compute_aware_groups is not None:
                    for ubid, group in enumerate(
                        compute_aware_groups
                    ):
                        cm_ubatch = _group_ascend_attn_metadata(
                            group,
                            cm,
                        )
                    ...
                    for ubid, cm_ubatch in enumerate(
                        _split_ascend_attn_metadata(ubatch_slices, cm)
                    ):
                        _build_attn_group_metadata(
                            kv_cache_gid, attn_gid, cm_ubatch,
                            cm_ubatch.num_reqs,
                            prefill_ratio_to_sas_metadata,
                            decode_ratio_to_sas_metadata,
                            common_ratio_to_sas_metadata,
    ...
    def initialize_attn_backend(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        def create_attn_groups(
            ...
            num_metadata_builders = self._metadata_builder_count()
