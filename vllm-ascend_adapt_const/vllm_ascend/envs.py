...
env_variables: dict[str, Callable[[], Any]] = {
    ...
    # Whether to record PP stage timing and dual-card PP metrics.
    "VLLM_ASCEND_PP_TRACE": lambda: bool(int(os.getenv("VLLM_ASCEND_PP_TRACE", "0"))),
    # JSONL file path for PP timing records.
    "VLLM_ASCEND_PP_TRACE_FILE": lambda: os.getenv(
        "VLLM_ASCEND_PP_TRACE_FILE", "/tmp/vllm_ascend_pp_trace.jsonl"
    ),
    # Whether to execute PP forward as model-runner internal micro-batches.
    "VLLM_ASCEND_PP_MICROBATCH": lambda: bool(int(os.getenv("VLLM_ASCEND_PP_MICROBATCH", "0"))),
    # Number of PP micro-batches when VLLM_ASCEND_PP_MICROBATCH=1.
    "VLLM_ASCEND_PP_MICROBATCH_NUM": lambda: int(os.getenv("VLLM_ASCEND_PP_MICROBATCH_NUM", "2")),
    # PP micro-batch grouping policy. Valid values are "scom"
    # (shape-bucket-safe stage/communication-aware ordered mapping),
    # "compute_aware" (equal-token scalar-cost grouping), and "uniform"
    # (the original contiguous equal-token slices). Non-sensitive.
    "VLLM_ASCEND_PP_MICROBATCH_GROUPING": lambda: os.getenv(
        "VLLM_ASCEND_PP_MICROBATCH_GROUPING", "scom"
    ),
    # Minimum scheduled tokens required before compute-aware grouping is
    # planned. Valid range: integer >= 0. Non-sensitive.
    "VLLM_ASCEND_PP_COMPUTE_AWARE_MIN_TOKENS": lambda: int(
        os.getenv("VLLM_ASCEND_PP_COMPUTE_AWARE_MIN_TOKENS", "512")
    ),
    # Minimum predicted critical-path gain, in percent, required to apply a
    # compute-aware plan. Valid range: integer from 0 to 100. Non-sensitive.
    "VLLM_ASCEND_PP_COMPUTE_AWARE_MIN_GAIN_PCT": lambda: int(
        os.getenv("VLLM_ASCEND_PP_COMPUTE_AWARE_MIN_GAIN_PCT", "5")
    ),
    # Token-block quantum used by the compute-aware planner. Valid range:
    # integer >= 1. Non-sensitive.
    "VLLM_ASCEND_PP_COMPUTE_AWARE_QUANTUM": lambda: int(
        os.getenv("VLLM_ASCEND_PP_COMPUTE_AWARE_QUANTUM", "8")
    ),
    # Minimum predicted full-pipeline gain, in percent, required to apply
    # an SCOM plan. Valid range: integer from 0 to 100. Non-sensitive.
    "VLLM_ASCEND_PP_SCOM_MIN_GAIN_PCT": lambda: int(
        os.getenv("VLLM_ASCEND_PP_SCOM_MIN_GAIN_PCT", "3")
    ),
    # Comma-separated, strictly increasing NPU token-shape bucket upper
    # bounds used by SCOM. Non-sensitive.
    "VLLM_ASCEND_PP_SCOM_SHAPE_BUCKETS": lambda: os.getenv(
        "VLLM_ASCEND_PP_SCOM_SHAPE_BUCKETS",
        "128,256,512,1024,2048,4096,8192",
    ),
    # Whether SCOM may search non-uniform capacities inside the safe shape
    # bucket. Valid values: 0 or 1. Non-sensitive.
    "VLLM_ASCEND_PP_SCOM_OPTIMIZE_CAPACITIES": lambda: bool(
        int(
            os.getenv(
    ...
    # Whether SCOM may cross the uniform baseline's shape bucket. This is
    # disabled by default because NPU operator costs jump at bucket edges.
    # Valid values: 0 or 1. Non-sensitive.
    "VLLM_ASCEND_PP_SCOM_ALLOW_BUCKET_CROSSING": lambda: bool(
        int(
            os.getenv(
    ...
    # Token quantum transferred between adjacent capacity candidates.
    # Valid range: integer >= 1. Non-sensitive.
    "VLLM_ASCEND_PP_SCOM_CAPACITY_QUANTUM": lambda: int(
        os.getenv("VLLM_ASCEND_PP_SCOM_CAPACITY_QUANTUM", "64")
    ),
    # Maximum number of bucket-safe capacity vectors evaluated per step.
    # Valid range: integer >= 1. Non-sensitive.
    "VLLM_ASCEND_PP_SCOM_MAX_CAPACITY_CANDIDATES": lambda: int(
        os.getenv(
            "VLLM_ASCEND_PP_SCOM_MAX_CAPACITY_CANDIDATES",
            "8",
        )
    ),
    # Maximum accepted adjacent member swaps in the SCOM local search.
    # Valid range: integer >= 0. Non-sensitive.
    "VLLM_ASCEND_PP_SCOM_MAX_SWAPS": lambda: int(
        os.getenv("VLLM_ASCEND_PP_SCOM_MAX_SWAPS", "4")
    ),
}
