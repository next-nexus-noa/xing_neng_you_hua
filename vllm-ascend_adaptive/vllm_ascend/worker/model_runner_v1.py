...
class NPUModelRunner(GPUModelRunner):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        self.adaptive_ubatch_controller: AdaptiveUBatchController | None = (
            AdaptiveUBatchController(
    ...
    def _select_adaptive_pp_microbatch_count(
        ...
        decision = self.adaptive_ubatch_controller.select(num_scheduled_tokens_np)
    ...
    def _commit_adaptive_pp_effective_m(
        ...
        *,
        ...
        if (
            self.adaptive_ubatch_controller is not None
            and get_pp_group().is_first_rank
        ):
            commit = getattr(
    ...
    def _sync_adaptive_pp_microbatch_decision(
        self,
        ...
        if pp.world_size <= 1:
            ...
            return decision
        payload = decision.to_payload() if pp.is_first_rank else None
        payload = pp.broadcast_object(payload, src=0)
        synced = AdaptiveUBatchDecision.from_payload(payload)
        if pp.is_first_rank and decision is not None:
            return decision
        return synced
    ...
    def _observe_adaptive_pp_microbatch_result(
        ...
        *,
        ...
        if (
            self.adaptive_ubatch_controller is None
            or decision is None
            or not get_pp_group().is_first_rank
        ):
            return
    ...
    def _flush_adaptive_pp_microbatch_result(self) -> None:
    ...
    def _should_measure_adaptive_pp_critical_path(
        ...
        return (
            self._adaptive_pp_feedback_step <= min_observations
            or self._adaptive_pp_feedback_step % interval == 0
            or decision.switched
            or decision.reason == "controlled_exploration"
        )
    ...
    def _observe_adaptive_pp_microbatch_failure(
        ...
        *,
        ...
        if self.adaptive_ubatch_controller is None or not get_pp_group().is_first_rank:
            return
    ...
    def _run_pp_microbatch_forward(
        ...
        *,
        ...
        for ubid, ubatch in enumerate(ubatch_slices):
            token_slice = ubatch.token_slice
            req_slice = ubatch.request_slice
            ...
            if pp.is_first_rank and intermediate_tensors is not None:
                mb_intermediate_tensors = intermediate_tensors[token_slice]

            mb_num_tokens = token_slice.stop - token_slice.start
            mb_num_reqs = req_slice.stop - req_slice.start
            mb_input_ids = self._slice_optional_tensor(input_ids, token_slice)
            mb_positions = self._slice_positions(positions, token_slice)
            if mb_positions.dim() == 1:
                update_cos_sin(mb_positions)
            mb_inputs_embeds = self._slice_optional_tensor(inputs_embeds, token_slice)
    ...
    def execute_model(
                ...
                if pp_microbatch_enabled:
                    if bool(getattr(self.parallel_config, "enable_adaptive_ubatch", False)):
                        ...
                        adaptive_pp_decision = (
                            self._sync_adaptive_pp_microbatch_decision(
                                local_decision
                            )
                        )
                        adaptive_pp_measure = (
                        ...
                        logger.info(
                            "Adaptive PP microbatch decision synced: "
        ...
        if adaptive_observe:
            if hasattr(torch, "npu") and torch.npu.is_available():
                torch.npu.synchronize()
            elapsed_ms = (
                time.perf_counter() - adaptive_pp_step_start
            ) * 1000.0
