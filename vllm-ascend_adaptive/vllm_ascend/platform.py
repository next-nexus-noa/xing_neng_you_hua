...
class NPUPlatform(Platform):
    ...
    def _fix_incompatible_config(vllm_config: VllmConfig) -> None:
        ...
        if vllm_config.parallel_config:
            ...
            adaptive_ubatch = getattr(
                vllm_config.parallel_config, "enable_adaptive_ubatch", False
            )
            pp_size = getattr(vllm_config.parallel_config, "pipeline_parallel_size", 1)
            if adaptive_ubatch and pp_size <= 1:
                logger.warning(
                    "'--enable-adaptive-ubatch' is only supported by the Ascend "
                    "PP micro-batch path for pipeline_parallel_size > 1. "
                    "Resetting to False."
                )
                vllm_config.parallel_config.enable_adaptive_ubatch = False
                adaptive_ubatch = False
            allow_ascend_pp_ubatch = pp_size > 1 and (
                adaptive_ubatch or ubatch_size > 1
            )
            if ubatch_size != 0 and not allow_ascend_pp_ubatch:
                logger.warning(
            ...
            elif allow_ascend_pp_ubatch:
                logger.info(
                    "Using Ascend PP micro-batch path with ubatch_size=%s, "
                    "adaptive_ubatch=%s.",
                    ubatch_size,
                    adaptive_ubatch,
                )
