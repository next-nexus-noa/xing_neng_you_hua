...
def communication_adaptation_310p():
    def broadcast310p_wrapper(fn):
        def broadcast310p(tensor, src=0, group=None, async_op=False, group_src=None):
            ...
            # IMPORTANT: Do NOT assign tensor_list[rank] = tensor here.
            # tensor may be an inference tensor (created under
            # torch.inference_mode()), and all_gather would perform an
            # inplace write into it, which PyTorch disallows outside
            # inference_mode. Keep the fresh tensor_list[rank] so
            # all_gather writes into a normal tensor.
            torch.distributed.all_gather(tensor_list, tensor, group=group)
            # Copy the result back into the original tensor under
            # inference_mode so the inplace update is permitted.
            with torch.inference_mode():
                tensor.copy_(tensor_list[src])
            if async_op:
                return NullHandle()
            else:
                return None
