...
MICROBATCH_POLICY="${MICROBATCH_POLICY:-${VLLM_ASCEND_PP_MICROBATCH_POLICY:-auto}}"
MICROBATCH_QUEUE_THRESHOLDS="${MICROBATCH_QUEUE_THRESHOLDS:-${VLLM_ASCEND_PP_MICROBATCH_QUEUE_THRESHOLDS:-0:1,4:2,16:4}}"
...
tail -n +2 "$PLAN" | while IFS=, read -r run_id scenario_id dataset scenario model_label model request_rate burstiness seed num_prompts micro_batch max_model_len host port pp_size tp_size dtype dataset_name dataset_path dataset_extra_args; do
  ...
  micro_args=()
  if [[ -n "$MICROBATCH_ARG_TEMPLATE" && "$micro_batch" != "0" && "$micro_batch" != "queue" ]]; then
  ...
  case "$MICROBATCH_POLICY:$micro_batch" in
    auto:queue)
      run_microbatch_policy="queue"
      ;;
    auto:*)
      run_microbatch_policy="fixed"
      ;;
    fixed:*|queue:*)
      run_microbatch_policy="$MICROBATCH_POLICY"
      ;;
    *)
      echo "Unknown MICROBATCH_POLICY=$MICROBATCH_POLICY; expected auto, fixed, or queue." >&2
      date -Is > "$run_dir/server.failed"
      continue
      ;;
  esac

  if [[ -n "$MICROBATCH_ENV_NAME" ]]; then
    if [[ "$MICROBATCH_ENV_NAME" == "VLLM_ASCEND_PP_MICROBATCH" ]]; then
      env_args+=("${MICROBATCH_ENV_NAME}=1")
    elif [[ "$micro_batch" == "queue" ]]; then
      env_args+=("${MICROBATCH_ENV_NAME}=queue")
    else
      env_args+=("${MICROBATCH_ENV_NAME}=$micro_batch")
    fi
  fi
  env_args+=("VLLM_ASCEND_PP_MICROBATCH_POLICY=$run_microbatch_policy")
  if [[ "$run_microbatch_policy" == "queue" ]]; then
    env_args+=("VLLM_ASCEND_PP_MICROBATCH_QUEUE_THRESHOLDS=$MICROBATCH_QUEUE_THRESHOLDS")
  fi
  ...
  if [[ "$micro_batch" != "0" && "$micro_batch" != "queue" ]]; then
...

  {
    ...
    echo "microbatch_policy=$run_microbatch_policy"
    echo "microbatch_queue_thresholds=$MICROBATCH_QUEUE_THRESHOLDS"
