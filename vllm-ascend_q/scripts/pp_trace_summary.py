...
def summarize_metrics(metrics: list[dict[str, Any]], skip: int) -> str:
    ...
    queue_lens = [x["queue_len"] for x in m if x.get("queue_len") is not None]
    selected_ms = [x["selected_m"] for x in m if x.get("selected_m") is not None]
    ...
    if queue_lens:
        lines.append(
            f"  queue_len range    : {min(queue_lens)}..{max(queue_lens)}"
        )
    if selected_ms:
        selected_counts = {
            selected: selected_ms.count(selected) for selected in sorted(set(selected_ms))
        }
        selected_counts_text = ", ".join(
            f"M={selected}: {count}" for selected, count in selected_counts.items()
        )
        lines.append(f"  selected_m counts  : {selected_counts_text}")
    if npu_metrics:
