...
class NPUModelRunner(GPUModelRunner):
    ...
    def _pp_microbatch_enabled(self) -> bool:
        return (
            (
                ...
                or os.getenv("VLLM_ASCEND_PP_MICROBATCH_POLICY", "fixed").lower() == "queue"
            )
            and get_pp_group().world_size > 1
        )

    def _pp_microbatch_configured_count(self) -> int:
        policy = os.getenv("VLLM_ASCEND_PP_MICROBATCH_POLICY", "fixed").lower()
        if policy == "queue":
            selected = getattr(self, "_queue_aware_pp_microbatch_num", None)
            if selected is not None:
                return max(1, int(selected))
        configured = int(os.getenv("VLLM_ASCEND_PP_MICROBATCH_NUM", "2"))
        return max(1, configured)

    def _pp_microbatch_queue_max_count(self) -> int:
        raw = os.getenv("VLLM_ASCEND_PP_MICROBATCH_QUEUE_THRESHOLDS", "0:1,4:2,16:4")
        max_count = 1
        for item in raw.split(","):
            ...
            if not item:
                continue
            try:
                _, micro_s = item.split(":", 1)
                max_count = max(max_count, int(micro_s))
    ...
    def _pp_microbatch_count(self, num_tokens: int) -> int:
    ...
    def _metadata_builder_count(self) -> int:
        ...
        if (
            ...
            if os.getenv("VLLM_ASCEND_PP_MICROBATCH_POLICY", "fixed").lower() == "queue":
                num_builders = max(num_builders, self._pp_microbatch_queue_max_count())
            else:
    ...
    def execute_model(
        ...
        queue_aware_m = getattr(scheduler_output, "pp_microbatch_num", None)
        if queue_aware_m is not None:
            self._queue_aware_pp_microbatch_num = int(queue_aware_m)
            logger.debug(
                "Queue-aware PP micro-batch received: queue_len=%s selected_m=%d",
                getattr(scheduler_output, "pp_queue_len", None),
                self._queue_aware_pp_microbatch_num,
            )
        ...
        with (
            ...
            with self.synchronize_input_prep():
                ...
                pp_microbatch_count = self.parallel_config.num_ubatches
                if pp_microbatch_enabled:
                    ...
                    should_ubatch = True
                    pp_microbatch_count = self._pp_microbatch_count(num_tokens_padded)
        ...
        with (
            ...
            if pp_microbatch_enabled and not get_pp_group().is_last_rank:
                self._finalize_dump_data()
                return None
