    ...
    def execute_model(
        ...
        if trace is not None:
            trace.begin_step(
                ...
                queue_len=getattr(scheduler_output, "pp_queue_len", None),
                selected_m=getattr(scheduler_output, "pp_microbatch_num", None),
            )
        ...
        if forward_pass and pp.world_size > 1 and self._pp_broadcast_comm is None:
            self._pp_broadcast_comm = _PPBroadcastCommunicator(self.vllm_config)
        use_runner_microbatch = (
            envs_ascend.VLLM_ASCEND_PP_MICROBATCH
            or envs_ascend.VLLM_ASCEND_PP_MICROBATCH_POLICY == "queue"
        )
        if forward_pass and not get_pp_group().is_first_rank and not use_runner_microbatch:
