...
class NPUPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        if ascend_config.enable_balance_scheduling:
            if ascend_envs.VLLM_ASCEND_PP_MICROBATCH_POLICY == "queue":
                raise ValueError(
                    "Queue-aware PP micro-batch is an M4 baseline and cannot be "
                    "combined with balance scheduling."
                )
            kv_transfer_config = vllm_config.kv_transfer_config
        ...
        if ascend_config.recompute_scheduler_enable:
            if ascend_envs.VLLM_ASCEND_PP_MICROBATCH_POLICY == "queue":
                raise ValueError(
                    "Queue-aware PP micro-batch is an M4 baseline and cannot be "
                    "combined with recompute scheduling."
                )
            kv_transfer_config = vllm_config.kv_transfer_config
            kv_role = getattr(kv_transfer_config, "kv_role", None)
        ...
        if ascend_config.SLO_limits_for_dynamic_batch != -1:
            if ascend_envs.VLLM_ASCEND_PP_MICROBATCH_POLICY == "queue":
                raise ValueError(
                    "Queue-aware PP micro-batch is an M4 baseline and cannot be "
                    "combined with SLO dynamic batch."
                )
        ...
        if ascend_config.profiling_chunk_config.enabled:
            if ascend_envs.VLLM_ASCEND_PP_MICROBATCH_POLICY == "queue":
                raise ValueError(
                    "Queue-aware PP micro-batch is an M4 baseline and cannot be "
                    "combined with profiling chunk scheduling."
                )
            vllm_config.scheduler_config.scheduler_cls = (
        ...
        if ascend_envs.VLLM_ASCEND_PP_MICROBATCH_POLICY == "queue":
            vllm_config.scheduler_config.scheduler_cls = (
                "vllm_ascend.core.scheduler_queue_aware_microbatch.QueueAwareMicrobatchScheduler"
            )
        elif ascend_envs.VLLM_ASCEND_PP_MICROBATCH_POLICY != "fixed":
            raise ValueError(
                "VLLM_ASCEND_PP_MICROBATCH_POLICY must be either 'fixed' or 'queue'."
            )
