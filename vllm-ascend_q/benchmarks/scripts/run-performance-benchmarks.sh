...
start_power_monitor() {
  ...
  if [[ "${POWER_METRICS_ENABLED:-1}" == "0" ]]; then
    return 0
  fi
  ...
  local monitor_args=(
    ...
    "sample"
    "--output" "$power_trace"
    "--interval-ms" "${POWER_METRICS_INTERVAL_MS:-500}"
  )
...
stop_power_monitor() {
  if [[ -n "${POWER_MONITOR_PID:-}" ]]; then
    ...
    echo "Power monitor stopped, pid=$POWER_MONITOR_PID"
    POWER_MONITOR_PID=""
  fi
}
...
run_serving_tests() {
  ...
  jq -c '.[]' "$serving_test_file" | while read -r params; do
    ...
    for qps in $qps_list; do
      ...
      power_trace="$RESULTS_FOLDER/${new_test_name}_power.jsonl"
      result_json="$RESULTS_FOLDER/${new_test_name}.json"

      client_command="vllm bench serve \
        --save-result \
        --result-dir $RESULTS_FOLDER \
        --result-filename ${new_test_name}.json \
        --power-metrics-file $power_trace \
        --request-rate $qps \
        $client_args"
      ...
      if [[ -s "$power_trace" && -f "$result_json" ]]; then
        python3 ../scripts/power_monitor.py report \
          --input "$power_trace" \
          --benchmark-json "$result_json" \
          --output "$result_json" || true
      fi
      set -e
